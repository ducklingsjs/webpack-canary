...
const containsSuccessMessage = function(stdout) {
  return stdout.indexOf('webpack: bundle is now VALID') > -1 ||
         ...
         stdout.indexOf('ce21cbdd9b894e6af794813eb3fdaf60.png  119 bytes') > -1;
};
...
export default function(configPath, completeExample) {
  ...
  const defaultCompilerCommand = `${path.relative(exampleDirectory, '.')}${webpack} --config ./webpack.config.js`;
