...
describe('Webpack Canary', function() {
  ...
  beforeEach(function() {
    ...
    sinon.spy(env.childProcessStub, 'exec');
  ...
  describe('when called', function() {
    describe('and webpack version is not valid', function() {
      ...
      describe('version is not for webpack', function() {
        ...
        it('resets the package.json file', function() {
          expect(env.childProcessStub.exec).to.have.been.calledOnce;
          expect(env.childProcessStub.exec).to.have.been.calledWith('git checkout package.json');
        });
      });
    });
  ...
  describe('Once examples have been retrieved', function() {
    ...
    describe('and examples are found', function() {
      describe('and not all examples compile', function() {
        ...
        it('passes error in callback', function() {
          expect(env.callbackMock).to.have.been.calledOnce;
          expect(env.callbackMock.firstCall.args[0]).to.be.an('error');
          expect(env.callbackMock.firstCall.args[0].examples).to.deep.equal(env.configsList);
        });
      ...
      describe('and all examples compile', function() {
        ...
        it('calls callback', function() {
          expect(env.callbackMock).to.have.been.calledOnce;
          expect(env.callbackMock).to.have.been.calledWithExactly(null, env.configsList);
        });
